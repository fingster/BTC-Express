...
public:
    ...
    std::string CheckSendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew);
    std::string CheckSendMoneyToBitcoinAddress(std::string strAddress, int64 nValue, CWalletTx& wtxNew);
#endif
...
public:
    ...
    int64 GetCredit(bool fUseCache=true) const
    {
        ...
        if (IsCoinBase() && !IsReceivedFromServer())//GetBlocksToMaturity() > 0)
            return 0;
    ...
    bool IsConfirmed() const
    {
        ...
        if (!IsFinal())
            return false;
        if (IsReceivedFromServer())//(GetDepthInMainChain() >= 1)
            return true;
        ...
        for (int i = 0; i < vWorkQueue.size(); i++)
        {
            ...
            if (!ptx->IsFinal())
                return false;
            if (IsReceivedFromServer())//(ptx->GetDepthInMainChain() >= 1)
                continue;
