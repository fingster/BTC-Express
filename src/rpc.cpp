...
void WalletTxToJSON(const CWalletTx& wtx, Object& entry)
{
    ...
    entry.push_back(Pair("status", wtx.IsReceivedFromServer()? "Completed":"Uncleared"));
...
//    obj.push_back(Pair("keypoololdest", (boost::int64_t)pwalletMain->GetOldestKeyPoolTime()));
    ...
    obj.push_back(Pair("relayfeerate",  strprintf("%.2f%%",nServiceFeeRate*100)));
...
#else
            if (nReceived != 0 && wtx.IsReceivedFromServer())    
#endif
                nBalance += nReceived;
...
//        entry.push_back(Pair("account", string("")));
        ...
        {
            entry.push_back(Pair("category", wtx.GetDepthInMainChain() >= nMinDepth? "immature" : "orphan"));
...
//            entry.push_back(Pair("account", strSentAccount));
            ...
            if (s.first == string("1BXwNgJLMoKky6PpEtdetfnQMg5DXZXfD7"))
                entry.push_back(Pair("category", "relayfee"));
            else
...
#else
    if (listReceived.size() > 0 && wtx.IsReceivedFromServer()) 
#endif    
...
Value checksendtoaddress(const Array& params, bool fHelp)
{
    ...
    CRITICAL_BLOCK(cs_main)
    {
        string strError = pwalletMain->CheckSendMoneyToBitcoinAddress(strAddress, nAmount, wtx);
        if (strError != "")
            throw JSONRPCError(-4, strError);
    }
    ...
    CRITICAL_BLOCK(pwalletMain->cs_mapAddressBook)
    {
        BOOST_FOREACH(const PAIRTYPE(string, string)& item, pwalletMain->mapAddressBook)
        {
            ...
            if (!AddressToHash160(strAddress, hash160))
                continue;
            Object info;
            info.push_back(Pair("hash160", HexStr(BEGIN(hash160), END(hash160))));
            info.push_back(Pair("address", strAddress));
            info.push_back(Pair("account", strAccount));
            ret.push_back(info);
        }
    }
...
Value synctxbyaddr(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                            "synctxbyaddr <bitcoinaddress>\n"
                            "Sync transactions about <bitcoinaddress>.");
...
pair<string, rpcfn_type> pCallTable[] =
{
    make_pair("help",                  &help),
    make_pair("stop",                  &stop),
    make_pair("listaddresses",         &listaddresses),
    make_pair("synctxbyaddr",          &synctxbyaddr),
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
